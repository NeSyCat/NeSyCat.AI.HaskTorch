...
module Examples.Binary.D_Grammatical.Formulas
  ( binaryPredicate,
    binarySentence,
  )
where
...
import Lib.B_Logical.LogicalQuantSignature (LogicalQuantSignature (..), Guard)
import Lib.B_Logical.LogicalSignature (LogicalSignature (..))
import Examples.Binary.C_Domain.Signature (BinaryRel (..), BinaryKlRel (..), BinarySorts (..))
