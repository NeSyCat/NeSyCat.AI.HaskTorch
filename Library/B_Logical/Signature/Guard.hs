...
--   (e.g. @Guard Dist a = [a]@, @Guard LogTens a = a@). Shared by every signature that has
--   aggregations. Keyed on the monad itself -- the guard genuinely differs by monad (a list to
--   fold for @Dist@, the batched tensor for @LogTens@), so the key moves from tag to monad.
