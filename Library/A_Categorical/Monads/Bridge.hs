...
--   'LogTens' (finitely-supported NON-normalized log-space measures) -- the ONLY inter-monad
--   structure in the framework:
--
--     decode :: LogTens a -> Dist a          -- softmax a (single-row) leaf      (the READING, per example)
--     encode :: [a] -> Tensor -> LogTens a   -- log a @[B,k]@ probability tensor (the EMBEDDING, batched)
--
...
import qualified A_Categorical.Monads.LogTens as LV
import qualified Torch
import qualified Torch.Functional as F

-- | Read a LogTens leaf out as a probability distribution: softmax the (first row of the) log-weights
--   over the leaf's own support. The @Dist@ READING / the readout, per example. Partial: a single
--   'LV.LogLeaf' (the net's logit leaf).
decode :: LogTens a -> Dist a
...
--   the @LogTens@ leaf of log-weights @log((1 - eps) p + eps)@. The @Dist => LogTens@ monad morphism,
--   batched -- the single embedding op the examples use for observations.
encode :: [a] -> Torch.Tensor -> LogTens a
encode xs probs =
  let eps = 1e-13 :: Float
      clamped = (probs `Torch.mul` Torch.asTensor (1 - eps :: Float)) `Torch.add` Torch.asTensor (eps :: Float)
   in LV.LogLeaf xs (Torch.log clamped)

-- | The @Dist => LogTens@ section as a monad morphism (the 'enc' of the section--retraction square):
--   transport the free-monad structure, a finite distribution to its log-weight leaf. On a certain
--   value @eta x = pure x@ it is just @pure x@ in 'LogTens' -- the case used by a two-sided neural
--   symbol  @decode . dig\@LogTens theta . encDist@.
encDist :: Dist a -> LogTens a
