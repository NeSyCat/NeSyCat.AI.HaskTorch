...
--   Each symbol's function lives in "C_Domain.Library.*"; nothing here inlines
--   @Torch.*@. To add a learnable layer: a 'Layer' symbol + a 'LayerWeight'
--   constructor + a "C_Domain.Library.*" function + one line in each table.
...
import C_Domain.Library.Activation.ELU (elu)
import C_Domain.Library.Activation.ReLU (relu)
import C_Domain.Library.Activation.Sigmoid (sigmoid)
import C_Domain.Library.Parameterized.Conv2d (conv2d, sampleConv2d)
import C_Domain.Library.Parameterized.Linear (linear, sampleLinear)
import C_Domain.Library.Shape.Flatten (flatten)
import C_Domain.Library.Shape.MaxPool (maxPool)
