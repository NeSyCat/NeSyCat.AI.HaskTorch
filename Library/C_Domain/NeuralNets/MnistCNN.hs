...
--   then 256 -> 100 -> 84 -> 10. Matches LTN's @SingleDigit@ (ELU convs + two dense heads)
--   exactly, so the MNIST-addition benchmark uses the same digit net as LTN/DeepProbLog.
module C_Domain.NeuralNets.MnistCNN (cnnArch, cnn) where
