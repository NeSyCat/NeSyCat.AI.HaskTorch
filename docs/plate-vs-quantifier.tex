...
\begin{itemize}
  ...
  \item \code{LogTens}: \textbf{vectorize-and-mean} --- apply the formula once to the batched carrier and
...
\begin{itemize}
  ...
  \item \textbf{Add} two small reducers: \code{forallExact} (the \code{Dist} enumerate-and-meet) and
        \code{plateMean} (the \code{LogTens} vectorize-and-mean / NLL). \emph{Only the \code{LogTens} one is
...
\code{LogTens} & \code{let (ln,ld)=logNumDen (phi g) in LogReduced (mean ln) (mean ld)} & \textbf{vectorize-and-mean} (NLL) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \textbf{The shared thing is the \emph{domain}, not batching.} Both readings quantify over the
        ...
        axis --- is a \code{LogTens}-only implementation detail. \code{Dist} does \textbf{not} batch; it
        iterates the dataset as a list. So ``plate'' correctly describes only the \code{LogTens} training
  ...
  \item \textbf{The aggregation differs, and that is the real conflation.} \code{Dist} computes the exact
        lattice meet (a hard Boolean: holds-for-all); \code{LogTens} computes the mean log-score (the
...
\code{Guard Dist a = [a]} (a list to traverse) vs \code{Guard LogTens a = a} (the fused \code{[B,k]} batch
...
\begin{enumerate}
  ...
  \item \textbf{Inner choices are already handled by \code{<-}, not by a nested quantifier.} Every latent
        ...
        log-space convolution for \code{LogTens}).
  \item \textbf{Three of the four quantifier methods are dead.} \code{bigVee}, \code{bigOplus},
        \code{bigOtimes} are \code{error "...not supported"} on the \code{LogTens} training path
...
\begin{itemize}
  ...
  \item In \code{LogTens} there is \textbf{no traversal at all}: \code{logNumDen (phi g)} applies the formula
...
\code{TwoMonBLat} operator $\Int(Q)$ --- and in the \code{LogTens} reading they cannot even express the
...
-- LogTens (the 'sat', differentiable training) -- vectorize-and-mean over the batch:
mnistAxiomTens :: Weights -> (Image, Image, LogTens Natural) -> LogTens Bool
mnistAxiomTens theta batch = plateMean (mnistFormula @LogTens theta batch)
...
-- LogTens: vectorize-and-mean. The 'plate' (i.i.d. batch mean) = the NLL = product t-norm (log space).
plateMean :: LogTens Bool -> LogTens Bool
plateMean prog =
...
\code{plateMean} is a verbatim move of the old \code{LogTens} \code{bigWedge} body, so \textbf{autograd and
...
\begin{enumerate}
  ...
  \item \textbf{Edit} \code{TensorBool.hs}: drop the \code{Guard} import, delete
        \code{type instance Guard LogTens a = a} and the whole \code{instance A2MonBLat LogTens Bool} (incl.\
  ...
  \item \textbf{Edit} the four \code{Examples/*/D\_Grammatical/Interpretation.hs}:
        \code{*AxiomTens = plateMean (*Formula @LogTens theta batch)} and
  ...
  \item \textbf{Untouched (verify, don't edit):} \code{NegLogSat.hs}, \code{InferenceInterpretation.hs},
        ...
        \code{TwoMonBLat.hs}. \code{sat} still returns \code{LogTens Bool}, so the objective
...
\begin{itemize}
  \item \textbf{Reader-monad transformer $B \circ m$} (18/30, most paper-faithful): models the plate as
        \code{PlateT m a = BatchIx -> m a}, the literal $B \circ m$. But its \code{LogTens} instance is a
...
\begin{itemize}
  ...
  \item \textbf{Dist path is dead code today:} \code{*AxiomData} is documented ``not called at runtime'';
        the only live consumer of the deleted \code{A2MonBLat} was the \code{LogTens}/training path. Grep for
