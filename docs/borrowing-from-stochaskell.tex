...
\code{Dist}/\code{Giry}/\code{LogTens} are free-monad GADTs (\code{A\_Categorical/Monads/}) &
...
one monad-polymorphic formula $\to$ \code{@Dist}/\code{@LogTens}, ported to JAX/PyTorch &
...
folds it.'' That is the same shape as our \code{collectLeaves} flattening a \code{LogTens} bind chain
...
\code{LogTens}/formula AST (not by probing a closure); a contraction-order chooser computed once per
...
$\eta$-lifted certain values (a one-hot \code{encode} \code{LogTens} leaf, bound \code{s <- n}).
...
is \textbf{uncertain} observation --- a likelihood weight per element, which in \code{LogTens} is just
