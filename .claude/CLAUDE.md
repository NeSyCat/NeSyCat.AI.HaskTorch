...
- **Sorts are monad-INVARIANT plain types** (`Image`/`Digit`/`Natural`/`Point`/`Omega = Bool`) — there is no per-monad sort assignment and no representation bridge for them (an image is the same tensor in both readings). The ONLY monad-dependent symbols are the **neural Kleisli symbols** — `digit` is a Kleisli *function* (`Image → m Digit`, lands in a sort; class `MnistKlFun`), while `classifierA`/`labelA` are Kleisli *relations* (land in `Omega = Bool`; class `BinaryKlRel`) — a class over `m` whose monad is `Dist` vs `LogVec`. The parameter space is the **pure parameters** `Weights` (θ), exposed as `Params = Weights`; the C *interpretation* picks the architecture by **importing** it and reaches the net via its named forward (`mlp θ` / `cnn θ`), with `initParams = sampleWeights mlpArch` (`cnnArch`) drawing θ₀. These symbols take **just θ** — never an architecture or a forward — and `Randomizable`/`sample` stays hidden inside `sampleWeights` (no `Spec` in the contract).
