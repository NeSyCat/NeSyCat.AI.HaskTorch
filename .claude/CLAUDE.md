...
- **`Library/`** — the shared, reusable framework, keeping the A–G layer names:
  ...
  - `C_Domain.Models.*` — reusable models as **architecture-as-data**, split signature/interpretation like `B_Logical`. `C_Domain.Models.Sequential.Signature` is the abstract vocabulary (`data Layer = Linear … | Conv2d … | ELU | ReLU | Sigmoid | MaxPool | Flatten`; `type Arch = [Layer]`; combinators `linearL`/`conv2dL`/`reluL`/`(>>>)`). `C_Domain.Models.Sequential.Interpretation` gives it meaning: `Weights` = θ (the **pure** sampled parameters — a `[LayerWeight]`, `Parameterized`, no forward/architecture inside), `sampleWeights :: Arch -> IO Weights` (draw θ₀), `runArch :: Arch -> Weights -> Tensor -> Tensor` (the forward), plus a second pure interpretation `countParams :: Arch -> Int`. `C_Domain.Models.Interpretations.{MLP,MnistCNN}` each export a named `Arch` (`mlpArch`/`cnnArch`) **and its forward at θ** (`mlp`/`cnn`, defined `= runArch <arch>`), so a call site writes `cnn θ x` instead of `runArch cnnArch θ x`. To add a learnable layer: a `Layer` symbol + a `LayerWeight` constructor + a case in `sampleWeights`/`runArch`.
