...
-- | Inference layer (F) for the MNIST example: the training config and the
--   objective, plus MNIST's interpretation of the (fixed) inference signature's
--   knowledge loss. The signature stays the same as binary's; only the
--   interpretation differs -- here the penalty is the categorical
--   negative-log-likelihood over the 19 sum classes, where binary interprets it
--   as 'softplus'. This is the single seam that turns the GeomU sum-logits into a
--   learning signal.
--
...
module MnistAddition.F_Inferential (objective, trainConfig, mnistKnowLoss) where
...
-- | The loss the trainer minimizes: the inference penalty (categorical NLL) of
--   the GeomU sum-logits term over the observed sums. Reaches the net only
--   through the grammatical term @digit(x)+digit(y)@ ('mnistSumLogits').
