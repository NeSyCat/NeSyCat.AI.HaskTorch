...
--   (probability) or @LogTens@ (differentiable). Interpreted per monad in
...
--   probability for @Dist@, the log-space convolution for @LogTens@).
mnistFormula ::
  forall m.
  (MnistKlFun m) =>
  Weights ->
...
--   (a list of triples in @Dist@, the batched triple in @LogTens@); the universal is 'bigWedge'.
mnistSentence ::
  forall m.
  ( MnistKlFun m,
    TwoMonBLat Omega,
