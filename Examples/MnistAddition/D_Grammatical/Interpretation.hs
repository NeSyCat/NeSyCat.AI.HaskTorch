...
--     LogTens (TENS):   one batch @(xs,ys,ns)@-> mean  @encode@(one-hot) (the batched @eta@, a leaf)
--
--   In BOTH, @n@ is the certain observation as a monadic value (@eta n@), bound with @s <- n@; the
--   @LogTens@ realization of @eta@ for a batch of certain sums IS the one-hot leaf (the encode). The
--   @Dist@ reading is the (non-differentiable) probability reading; the @LogTens@ reading is the 'sat'.
...
import MnistAddition.C_Domain.Interpretation () -- the MnistKlFun instances
...
-- | The @LogTens@ reading (differentiable training, the 'sat'): a pure pass-through. The batch
--   ALREADY carries the observed sum as @eta n@ (a @LogTens Natural@ leaf, built by the E layer),
--   so there is no lifting here -- just read the sentence at @\@LogTens@ over the batch.
mnistAxiomTens :: Weights -> (LogTens Image, LogTens Image, LogTens Natural) -> LogTens Bool
mnistAxiomTens theta batch = mnistSentence @LogTens () batch theta
