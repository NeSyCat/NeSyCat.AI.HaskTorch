...
--   @Examples/MnistAddition/G_Data/get-mnist.sh@); this module only reads them and forms the fixed
...
module MnistAddition.G_Data.Loader
  ( MnistDataset (..),
    loadMnistDataset,
  )
where
...
-- | Directory holding the four gzipped IDX files (see @Examples/MnistAddition/G_Data/get-mnist.sh@).
mnistDir :: String
mnistDir = "Examples/MnistAddition/G_Data"
...
loadMnistDataset = do
  ...
  unless (and present) $
    die ("MNIST data not found in " ++ mnistDir ++ "/. Run:  Examples/MnistAddition/G_Data/get-mnist.sh")
