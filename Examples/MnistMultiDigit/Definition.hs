...
--   file. Reuses everything single-digit does (the LogVec monad, Boolean/TensorBool logic, the
...
-- B (logic) — REUSE template: the library Boolean (Dist quantifier) + TensorBool (LogVec quantifier).
...
-- F (inference) — REUSE the library's shared @instance InferenceSignature (LogVec Bool)@;
...
instance Example MnistMultiDigit where
  ...
  type Batch MnistMultiDigit = (Torch.Tensor, Torch.Tensor, Torch.Tensor, Torch.Tensor, LogVec Int)
  type Truth MnistMultiDigit = LogVec Bool
