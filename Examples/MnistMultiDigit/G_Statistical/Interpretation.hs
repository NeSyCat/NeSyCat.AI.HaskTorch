...
--   against true labels). Predictions are the argmax of the @LogTens@ logits (no softmax); a
...
predDigits theta imgs =
  Torch.asValue (Torch.argmax (Torch.Dim 1) Torch.RemoveDim (logTensLeafTensor (digit @LogTens theta imgs)))
