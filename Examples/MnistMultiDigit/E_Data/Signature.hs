...
--   already as @eta n@ -- a @LogTens Int@ leaf over [0..198]); the @*Img@/@*Lab@/@*Sums@ fields back
...
-- | A dataset of MNIST image quadruples and their observed sums (the sum as a @LogTens Int@ leaf).
--   Each @*Img@ field is a @[N,1,28,28]@ batch; the four digit-label lists back per-digit accuracy.
data MultiDataset = MultiDataset
  { trainBatch :: (Torch.Tensor, Torch.Tensor, Torch.Tensor, Torch.Tensor, LogTens Int),
