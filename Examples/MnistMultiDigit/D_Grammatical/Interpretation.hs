...
--   The @Dist@ reading is the (non-differentiable) probability reading; the @LogVec@ reading is the
--   'sat'. The @LogVec@ reading is a pure pass-through: the batch ALREADY carries the observed sum
--   as @eta n@ (a @LogVec Natural@ leaf over [0..198], built by the E layer), so there is no lifting
--   here.
...
-- | The @LogVec@ reading (differentiable training, the 'sat'): a pure pass-through over the batch,
--   which already carries the observed sum as @eta n@ (a @LogVec Natural@ leaf, built by the E
...
multiAxiomTens :: Weights -> (Image, Image, Image, Image, LogVec Natural) -> LogVec Bool
multiAxiomTens theta batch = multiSentence @LogVec () batch theta
