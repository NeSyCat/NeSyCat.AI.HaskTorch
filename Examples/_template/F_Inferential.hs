-- | Inference layer (F) for the Template example: the training config and the
--   objective. This stub REUSES 'softplus' from the library and, for simplicity,
--   maps the model output directly through the penalty; in a real example route
--   the penalty through the D (grammatical) axiom over the data (see
--   "Binary.F_Inferential" / "MnistAddition.F_Inferential").
module Template.F_Inferential (objective, trainConfig, templateLoss) where
