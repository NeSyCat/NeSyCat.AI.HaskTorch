...
--     LogVec (TENS):   one batch pair      -> mean     @encode@ (one-hot leaf over the batch's
--                                                       distinct pairs -- built by the E layer)
--
--   In BOTH, the observation is the certain pair as a monadic value (@\eta (ns, y)@), bound
--   @(ns, y) <- obs@ exactly like the sketch decisions; the @LogVec@ realization of @\eta@
...
-- | The @LogVec@ reading (differentiable training, the 'sat'): a pure pass-through. The
--   batch ALREADY carries the observation as @\eta (ns, y)@ (a @LogVec (Numbers, Answer)@
--   leaf over the batch's distinct pairs, built by the E layer), so there is no lifting
--   here -- just read the sentence at @\@LogVec@ over the batch.
wapAxiomTens :: Weights -> ([Problem], LogVec (Numbers, Answer)) -> LogVec Bool
wapAxiomTens theta batch = wapSentence @LogVec () batch theta
