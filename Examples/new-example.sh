...
# Examples/<Name>/, renames the placeholder, and registers it in the dispatcher.
# After this, `hpack` (run automatically) picks up the new modules with no manual
# cabal edit. Usage:  Examples/new-example.sh <Name>   (UpperCamelCase)
set -euo pipefail
...
# The script lives in Examples/; the repo root is one level up.
...
NAME="$NAME" KEBAB="$KEBAB" perl -i -pe '
  s{^-- NEW-EXAMPLE-IMPORT}{import $ENV{NAME}.Example ($ENV{NAME})\n-- NEW-EXAMPLE-IMPORT};
  s{^  -- NEW-EXAMPLE-ARM}{  ("$ENV{KEBAB}" : rest) -> runExample \@$ENV{NAME} (parseN rest)\n  -- NEW-EXAMPLE-ARM};
' "$ROOT/Library/Run.hs"

# Regenerate the cabal so the new modules are built (hpack globs Library/ + Examples/).
( cd "$ROOT" && ~/.cabal/bin/hpack >/dev/null 2>&1 || hpack >/dev/null 2>&1 || \
  echo "note: run hpack manually (not found on PATH)" >&2 )

echo "Created Examples/$NAME/ (full A-G stack) and registered '$KEBAB'."
