...
--     LogTens (TENS):  the guard is one BATCHED @[B,2]@ tensor        -> one vectorized op
--
...
--   reading is the (non-differentiable) probability reading; the @LogTens@ reading is the 'sat'.
...
-- | The @LogTens@ reading (differentiable training, the 'sat'): the guard is ONE batched @[B,2]@
--   tensor of points, handled in a single vectorized op.
binaryAxiomTens :: Weights -> Torch.Tensor -> LogTens Omega
binaryAxiomTens theta guard = binarySentence @LogTens () guard theta
