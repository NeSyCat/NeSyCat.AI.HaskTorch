...
--   probabilistically (M = Dist). Guard is @[Point] = [Torch.Tensor]@ -- a finite subset of
--   R^2 (each a [2] tensor). Mirrors "Binary.D_Grammatical.InterpretationTens" (theta, data).
binaryAxiomData :: Weights -> [Point] -> M MeasU Omega
